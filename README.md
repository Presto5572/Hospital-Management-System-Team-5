...
Query 6:
Query: 3 show which wards have had the most appointments
Why?: Can be used to see which wards may need upgrades or maitenance the most
...
Query 7:
Query: Show the the average costs associated with each diagnosis that are above the average costs of all appointments
Why?: Can help a hospital see if they are overspending for certain visits so they can start to figure out where to cut costs
...
Query 10:
Query: Show given diagnosises that did not lead to a prescription
Why?: Past decisions made by doctors can help newer doctors understand why to do in certain scenarios
